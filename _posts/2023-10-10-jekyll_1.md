...
+ [开发环境搭建](#开发环境搭建)
    ...
    + [安装 jekyll](#安装-jekyll)
    + [运行 jekyll](#运行-jekyll)
+ [配置 github 的 githubPages 服务](#配置-github-的-githubpages-服务)
...
### 配置 github 的 githubPages 服务
如果 github 没有代码仓，先创建一个代码仓，静态页服务的代码仓一定要是``public``类型。

![blogs-18](/static/img/blogs/blogs-18.jpg)

![blogs-19](/static/img/blogs/blogs-19.jpg)

![blogs-20](/static/img/blogs/blogs-20.jpg)

![blogs-20](/static/img/blogs/blogs-21.jpg)
> 由于 ``github`` 存在 ``dns`` 污染问题，访问经常有问题，可以在 ``gitee`` 码云开启静态页服务进行日常开发，开发完成后再把代码推送到 ``github`` 上。具体可以参考博客中关于 ``git`` 的教程中如何用 ``A`` 平台代码仓，初始化 ``B`` 平台的代码仓并推送代码到 ``B`` 平台。