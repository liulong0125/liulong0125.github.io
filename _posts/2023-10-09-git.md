...
### 场景二: 先写代码后创建的代码仓代码推送
代码开发完了后，代码仓后创建的（已经初始化了，如果未初始化直接 git remote add origin git@xxx 关联后就可以直接推送了，参考该文章介绍的 "远程仓的使用"）初次推送的时候可以使用
``git pull origin master --allowunrelated-histories -u`` 进行代码推送。
