...
1. 创建秘钥：``git bash`` 中输入 ``ssh-keygen -t rsa -C 'tom@163.com' -f ~/.ssh/codeup_id_rsa`` 通过手动指定生成的秘钥名称前缀，避免覆盖掉之前平台的秘钥。
...
5. ``git commit --allow-empty -m "初始化"``。
...
### 场景九: 变基 rebase
[官网文档1](https://git-scm.com/docs/git-rebase/zh_HANS-CN)
[官网文档2](https://git-scm.com/book/zh/v2/Git-分支-变基)
