...
+ 题外话，当我们需要打印一张 A4 纸大小的图片的时候，创建画板宽：``21cm`` 高 ``29.7cm``（印刷尺 ``A4`` 高度为  ``28.5cm`` )，这个时候显示分辨（``ppi``）会设置为``300``，因为最终要打印，打印机的打印分辨率``dpi``是``300``，如果显示分辨率设置的远小于打印分辨率(``dpi``)，会造成图片模糊，如下：<br />![03](/static/img/mediaQuery/03.jpg)
